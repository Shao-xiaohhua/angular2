import { Component, OnInit, DoCheck, ViewChild, EventEmitter, ElementRef, Input } from '@angular/core';
...
export class ProjectDataComponent implements OnInit, DoCheck {
  ...
  private removeGroup (isSubmit): void {
    if (isSubmit) {
  ...
  private printStuff() {
    window.open(environment.serverUrl + '/print/print.jsp?ids=' + JSON.stringify([this.currentStuff.id]) +
    '&typeId=' + this.currentStuff.typeId, '_blank');
  }
  ...
  private printAllStuff() {
    ...
    window.open(environment.serverUrl + '/print/print.jsp?ids=' + JSON.stringify(stuffIds) +
    '&typeId=' + this.currentStuff.typeId, '_blank');
  }
}
